# Filename Parser (Rust Port) Implementation Plan
...
> **Related Issues:** None found in research document.

**Goal:** Port Java's `FilenameParser.java` to `src-tauri/src/parser/mod.rs`, preserving identical behaviour for all 103 existing test cases.

> **Note on test count:** The research doc says "95 test cases" — that is the count of `setupValues01`–`setupValues95` numbered methods. The actual total is **103**: 6 named setup methods + 95 numbered + 2 extras (`setupValues02a` and `setupValues161`). All 103 are ported in this plan.

**Architecture:** 16 compiled regex patterns (8 base × 2: with/without resolution suffix), tried in order, first match wins. Helpers mirror Java's `stripJunk` and `insertShowNameIfNeeded` exactly. All logic lives in one file; the module is small enough that splitting adds no value.

**Tech Stack:** Rust `regex` crate (already in `Cargo.toml`), `std::sync::OnceLock` for lazy-compiled patterns, `std::path::Path` for directory traversal.

**Context Gathered From:**
- `docs/hyperpowers/research/2026-03-29-modernise-parser.md`
- `src/main/java/org/tvrenamer/controller/FilenameParser.java` (full source)
- `src/main/java/org/tvrenamer/controller/util/StringUtils.java` (helper logic)
- `src/test/java/org/tvrenamer/controller/FilenameParserTest.java` (all 95 cases)
- `src-tauri/Cargo.toml` (`regex = "1"` already present)

---

## Critical Design Notes (Read Before Writing a Line)

### Why `^` anchor on every pattern

Java's `Matcher.matches()` requires the **entire string** to match the pattern — equivalent to anchoring at both start and end. Since all 8 base patterns end with `.*`, only a `^` prefix is needed in Rust to replicate this.

Without `^`, Rust's `captures()` finds a substring match anywhere in the input, producing wrong results (e.g., matching season digits mid-string).

### Pattern compilation order

```
COMPILED_PATTERNS[0..7]  = BASE_PATTERNS[0..7] + RESOLUTION_SUFFIX  (tried first)
COMPILED_PATTERNS[8..15] = BASE_PATTERNS[0..7] alone                 (tried second)
```

Patterns 0–7 always produce 4 capture groups; patterns 8–15 produce 3. Use the pattern index (`i < 8`) to decide whether to extract a resolution group.

### `insert_show_name_if_needed` — the complex one

Java `FileEpisode` holds a `java.nio.file.Path`. The Rust API receives a raw `&str` (possibly a full path). This function:

1. Gets the **last path component** (filename only).
2. If it matches `FILENAME_BEGINS_WITH_SEASON`, climbs the parent directory tree, skipping dirs that look like season dirs or the special "versions" duplicates dir.
3. Returns `"<parent_name> <filename>"` so the regex patterns can extract the show name from the parent directory.

`DUPLICATES_DIRECTORY = "versions"` — confirmed from `Constants.java:207`. Any parent dir literally named `"versions"` is treated as a staging/duplicates folder and skipped.

`EXCESS_SEASON` regex strips season suffixes from directory names (e.g., `Quintuplets.Season01` → `Quintuplets`) so the show name isn't polluted.

`DIR_LOOKS_LIKE_SEASON = ^[sS][0-3]\d$` — Java `String.matches()` is a full-string match, so anchor both ends in Rust.

### `trim_found_show`

Strips leading and trailing `[ _.-]` from the captured show name. Direct port of Java's:
```java
return extracted.replaceFirst("^[ _.-]+", "").replaceFirst("[ _.-]+$", "");
```

### `remove_last_ci` (case-insensitive removeLast)

Java's `StringUtils.removeLast(input, match)` does a case-insensitive `lastIndexOf`, then removes that occurrence only if `idx > 0` (not at position 0). Rust's `rfind` on the lowercased string gives the same behaviour.

### Resolution extraction

The resolution suffix `\D(\d+[pk]).*` — capture group 4 in with-resolution patterns — matches strings like `720p`, `1080p`, `480p`, `4k`. The leading `\D` ensures it's preceded by a non-digit (separator).

### `ParseResult.show_name` vs Java's `filenameShow`

Both are the **raw extracted string** (after `trimFoundShow`) with no further normalisation. The Java test compares `retval.getFilenameShow()` directly to the literal string in `EpisodeTestData`. Use the same raw value in Rust.

### Integer parsing of season/episode

Java parses the captured string to `int` (e.g., `"01"` → `1`, `"105"` → `105`). Rust's `str::parse::<u32>()` does the same — leading zeros are dropped, which is what the tests expect.

---

## Task 1: Stub + Complete Test Suite (All 95 Cases)

**Files:**
- Modify: `src-tauri/src/parser/mod.rs`

Write the `ParseResult` struct and a `parse_filename` stub that always returns `None`. Then write all 95 `#[test]` functions so every test fails immediately with a clear error. This lets you run the suite and watch the count drop to zero as you implement.

- [ ] **Step 1: Write the stub**

Replace the entire contents of `src-tauri/src/parser/mod.rs` with:
...
- [ ] **Step 2: Add all 95 test functions inside the `tests` module**

The test function naming convention: `test_<setup_method_name>` (snake_case, matches Java `setupValuesXX` methods).

Resolution `None` = no `.episodeResolution(...)` call in Java. Resolution `Some("720p".to_string())` = `.episodeResolution("720p")` in Java.
...
// See "Critical Design Notes" for how this works.
...
- [ ] **Step 3: Run the test suite — expect 103 failures**

```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests 2>&1 | tail -20
```

Expected output includes:
```
test parser::tests::test_three_digits ... FAILED
...
test result: FAILED. 0 passed; 103 failed
```
...
git commit -m "test(parser): add all 95 filename parser tests (all failing)"
```

---

## Task 2: Data Structures and Compiled Patterns

**Files:**
- Modify: `src-tauri/src/parser/mod.rs`

- [ ] **Step 1: Replace the stub with full data + compilation code**

Add the following at the top of `mod.rs` (after `use` declarations), replacing the `parse_filename` stub with a still-returning-`None` version but now with all infrastructure:
...
- [ ] **Step 2: Verify patterns compile — run just the compilation check**
...
- [ ] **Step 3: Add a one-line test that forces pattern initialisation**

Inside the `tests` module, add before the existing tests:
...
- [ ] **Step 4: Run to confirm this new test passes and 103 still fail**

```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests 2>&1 | grep -E "^test result|PASSED|patterns_compile"
```

Expected: `patterns_compile ... ok`, everything else `FAILED`.

- [ ] **Step 5: Commit**

```bash
git add src-tauri/src/parser/mod.rs
git commit -m "feat(parser): add ParseResult struct and compile 16 regex patterns"
```
...
- [ ] **Step 1: Add `trim_found_show`**

Port of Java `StringUtils.trimFoundShow`:
```java
return extracted.replaceFirst("^[ _.-]+", "").replaceFirst("[ _.-]+$", "");
```

Add this function after the `OnceLock` block:
...
- [ ] **Step 2: Add unit tests for `trim_found_show`**
...
- [ ] **Step 3: Run `trim_found_show` tests**

```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests::trim_found_show 2>&1
```

Expected: all `trim_found_show_*` tests pass.

- [ ] **Step 4: Add `remove_last_ci` and `strip_junk`**

Port of Java `StringUtils.removeLast` (case-insensitive) + `FilenameParser.stripJunk`:
...
- [ ] **Step 5: Add unit tests for `strip_junk`**
...
- [ ] **Step 6: Run `strip_junk` tests**

```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests::strip_junk 2>&1 && cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests::remove_last 2>&1
```
...
```bash
git add src-tauri/src/parser/mod.rs
git commit -m "feat(parser): implement trim_found_show, remove_last_ci, strip_junk helpers"
```

---

## Task 4: Implement `insert_show_name_if_needed`

**Files:**
- Modify: `src-tauri/src/parser/mod.rs`

This is the most complex function. Read the "Critical Design Notes" at the top of this plan before starting.

- [ ] **Step 1: Add `extract_parent_name`**
...
fn extract_parent_name(path: &Path) -> Option<String> {
...
- [ ] **Step 2: Add `insert_show_name_if_needed`**
...
- [ ] **Step 3: Add unit tests for `insert_show_name_if_needed`**
...
- [ ] **Step 4: Run `insert_show_name_if_needed` tests**

```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests::insert_show_name 2>&1
```

Expected: all `insert_show_name_*` tests pass.

- [ ] **Step 5: Commit**

```bash
git add src-tauri/src/parser/mod.rs
git commit -m "feat(parser): implement insert_show_name_if_needed path traversal"
```

---

## Task 5: Implement `parse_filename` and Pass All Tests

**Files:**
- Modify: `src-tauri/src/parser/mod.rs`

- [ ] **Step 1: Replace the stub `parse_filename` with the real implementation**
...
```bash
cargo test --manifest-path src-tauri/Cargo.toml --lib parser::tests 2>&1
```

**Expected:** `test result: ok. 103+ passed; 0 failed`

If some tests fail, check the failure message carefully:
- **`show_name` wrong:** Check `insert_show_name_if_needed` path traversal (see Task 4 design notes) or `trim_found_show`.
- **`season`/`episode` wrong:** Verify which pattern matched (add a `dbg!` to the loop temporarily). Check pattern priority — a lower-numbered pattern is matching when a higher-numbered one should.
- **`resolution` wrong:** Verify pattern index < 8 check is correct. Check that `RESOLUTION_SUFFIX` is appended to the pattern correctly.
- **`None` returned:** The stripped string isn't matching any pattern. Print `&stripped` with `dbg!` to inspect what `strip_junk` and `insert_show_name_if_needed` produced.

Quick debugging template (do NOT commit this):
```rust
// Temporary — remove before committing
pub fn parse_filename_debug(input: &str) -> Option<ParseResult> {
    let with_show = insert_show_name_if_needed(input);
    let stripped = strip_junk(&with_show);
    dbg!(&stripped);
    // ... rest of function
...
- [ ] **Step 3: Once all 95 tests pass, commit**

```bash
git add src-tauri/src/parser/mod.rs
git commit -m "feat(parser): implement parse_filename — all 103 tests pass"
```
...
|------------|-------------|
| `regex = "1"` in `Cargo.toml` | Confirmed: `src-tauri/Cargo.toml` |
| `src-tauri/src/parser/mod.rs` exists (`mod parser;` in `lib.rs`) | Confirmed |
| `DUPLICATES_DIRECTORY = "versions"` | Confirmed: `Constants.java:207` |
| All 8 Java patterns — no lookahead/lookbehind | Confirmed: `FilenameParser.java:26-52` |
| `OnceLock` available (`rust-version = "1.77"`, requires ≥ 1.70) | Confirmed: `src-tauri/Cargo.toml` |
| Java test file has **103** cases (not 95 as research doc states) | Confirmed: 6 named + 97 numbered (01–95 + 02a + 161) |
| Java `Integer.parseInt("01") == 1` → Rust `"01".parse::<u32>() == Ok(1)` | Standard behaviour |

### ⚠️ Verify Before Relying On

| Assumption | Risk |
|------------|------|
| Rust `regex` crate handles lazy quantifiers (`\D*?`, `\d\d*`) identically to Java | Test patterns 3 and 5 carefully — they use lazy non-greedy quantifiers. If results are off, add `fancy_regex` to `Cargo.toml` and try `fancy_regex::Regex` instead. |
| `strip_junk` must only strip last occurrence (Java `removeLast` not `replaceAll`) | If multiple "hdtv" tokens appear, only the LAST is removed. The `rfind` in `remove_last_ci` handles this correctly. |
| `DIR_LOOKS_LIKE_SEASON` anchoring: `^[sS][0-3]\d$` | Java's `String.matches()` requires full-string match. The `$` anchor is essential — without it, "seasonal" would incorrectly match. Verify with `test_78` (complex Quintuplets path). |

### ❌ Known Gaps (Do Not Fix)

| Gap | Decision |
|-----|----------|
| Season 0 / TMDB special episodes | Not handled by any Java pattern; documented gap, not in scope |

---

## Quick Reference: Pattern → Test Cases

| Pattern | Compiled index | Representative tests |
|---------|---------------|---------------------|
| 1 SxxExx | 0 (with res), 8 (without) | `test_three_digits`, `test_07`, `test_17`–`test_38` |
| 2 Season-XX-Episode-XX | 1, 9 | `test_long_three_digit_season` |
| 3 sXX.eXX | 2, 10 | `test_dot_three_digit_season`, `test_02`, `test_03` |
| 4 SSxEE | 3, 11 | `test_01`, `test_41`–`test_54`, `test_161` |
| 5 year in title | 4, 12 | `test_04`, `test_14`, `test_25`, `test_83`–`test_90` |
| 6 SXXYY (4-digit after S) | 5, 13 | `test_56`, `test_57` |
| 7 fallback numeric | 6, 14 | `test_02a`, `test_05` |
| 8 last resort | 7, 15 | `test_95`, `test_40` |
