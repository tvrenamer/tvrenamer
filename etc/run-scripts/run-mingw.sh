...
loclibs="swt-win64-4.3.jar jedit-4.3.2-IOUtilities.jar commons-codec-1.4.jar xstream-1.3.1.jar xpp3_min-1.1.4.jar"
...
if [ -d ~/.tvrenamer ]
then
    /bin/mv ~/.tvrenamer/prefs.xml ~/prefs.xml
    /bin/rm -rf ~/.tvrenamer
    /bin/mv ~/prefs.xml ~/.tvrenamer
elif [ ! -f ~/.tvrenamer ]
then
  echo "Error: no ~/.tvrenamer file found"
fi
